...
from auth import discord_oauth, auth

def render():
    st.set_page_config(page_title="Login - Sinistra", layout="wide")
    # Custom CSS for login page and hide sidebar
    st.markdown("""
        <style>
            :root {
                --sinistra-red: #FF4B4B;
            ...
            .divider::before, .divider::after {
                content: '';
                flex: 1;
                border-bottom: 1px solid var(--sinistra-yellow);
            }
            .divider::before {
                margin-right: 10px;
            }
    ...
    if 'code' in query_params:
        with st.spinner("Processing Discord login..."):
            # The auth system will handle the OAuth callback and show appropriate messages
            pass
        return
    ...
    with st.container():     
        ...
        try:
            server_id = st.secrets["discord"]["server_id"]
            st.info(f"🎮 You must be a member of the CIU Discord Server to access this application.")
        except:
            st.warning("Discord server configuration not found.")
        ...
        try:
            ...
            discord_button = f"""
            <a href="{authorization_url}" class="discord-button" target="_self">
                <svg class="discord-logo" width="24" height="24" viewBox="0 0 71 55" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            """
            st.markdown(discord_button, unsafe_allow_html=True)
        except Exception as e:
            st.error(f"Error generating Discord login URL: {e}")
